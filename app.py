...
def index():
    ...
    deck_stats = []
    for d in decks:
        wins = GameParticipant.query.join(Game).filter(
    ...
    return render_template('index.html', player_stats=player_stats, deck_stats=deck_stats,
                           recent_games=recent_games, game_parts=game_parts)
...
def start_game():
    ...
    for i in range(1, 7):  # Up to 6 players
        ...
        if p_id and d_id:
            ...
            participants.append({'player_id': p_id, 'deck_id': d_id, 'player_name': deck.owner.name, 'deck_name': deck.name})
